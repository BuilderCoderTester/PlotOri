        ...
        <div className="container mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              {[1, 2, 3].map((s) => <div key={s} className="flex items-center gap-1">
                  <div
    className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all ${step === s ? "bg-[hsl(158,50%,28%)] text-white" : step > s ? "bg-[hsl(158,20%,90%)] text-[hsl(158,50%,28%)]" : "bg-slate-100 text-slate-400"}`}
  >
                    {step > s ? <CheckCircle2 className="w-4 h-4" /> : s}
                  </div>
                  <span
    className={`hidden sm:inline text-xs font-medium ${step === s ? "text-[hsl(158,50%,28%)]" : "text-slate-400"}`}
  >
                    {s === 1 ? "Activity" : s === 2 ? "Context" : "Draft"}
                  </span>
                  {s < 3 && <span className="text-slate-300 text-xs ml-1">›</span>}
                </div>)}
            </div>
          </div>
    ...
    /* Editor header */
                    ...
                    <div className="px-6 pt-6 pb-4 border-b bg-[hsl(158,20%,98%)]">
                      <div className="flex items-center gap-2 mb-1">
                        {activityType && <Badge className="bg-[hsl(158,20%,90%)] text-[hsl(158,50%,28%)] border-[hsl(158,20%,80%)] text-xs font-medium">
                            {activityType.label}
                          </Badge>}
